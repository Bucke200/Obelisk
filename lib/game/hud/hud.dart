...
import 'package:flame_simple_platformer/core/game_manager.dart';
...
class Hud extends Component with HasGameReference<SimplePlatformer> {
  ...
  Future<void> onLoad() async {
    ...
    GameManager.instance.score.addListener(onScoreChange);
    GameManager.instance.lives.addListener(onHealthChange);
  ...
  @override
  void onRemove() {
    GameManager.instance.score.removeListener(onScoreChange);
    GameManager.instance.lives.removeListener(onHealthChange);
    super.onRemove();
  }

  // Updates score text on hud.
  void onScoreChange() {
    scoreTextComponent.text = 'Score: ${GameManager.instance.score.value}';
  }

  // Updates health text on hud.
  void onHealthChange() {
    healthTextComponent.text = 'x${GameManager.instance.lives.value}';

    // Load game over overlay if health is zero.
    if (GameManager.instance.lives.value == 0) {
